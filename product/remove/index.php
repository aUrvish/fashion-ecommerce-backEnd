<?php
    ...
    if($res['id']){
        ...
        if(mysqli_query($conn, $sql) or die('Querry not run')){
            echo json_encode(array("message"=>"Product remove successfully" ,"data"=> array(), "error"=>false));
        }else {
            echo json_encode(array("message"=>"Product not remove" ,"data"=> array(), "error"=>true));
        }
    }else{
